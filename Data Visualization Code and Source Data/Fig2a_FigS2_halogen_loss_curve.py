...
# =====================================================
# This script visualizes training and validation loss curves by fold.
# The CSV file already contains preprocessed data
# =====================================================
...
# Load CSV file
file_path = r"C:\Users\13709\Desktop\论文撰写四\模型\训练验证\cl_br_bin_0.1_trainval_metrics.csv"
...
for fold in range(1, max_fold + 1):
    ...
    ax.plot(epochs, train_losses, linewidth=2, alpha=0.8, color='#FADCAE', label='Training Loss')
    ax.plot(epochs, val_losses, linewidth=2, alpha=0.8, color='#33A89A', label='Validation Loss')
    ...
    if max_epoch < 50:
        ...
        ax.plot(extended_epochs, [train_losses[-1]] * len(extended_epochs),
                'b--', linewidth=1.5, alpha=0.5, color='#FADCAE')
        ax.plot(extended_epochs, [val_losses[-1]] * len(extended_epochs),
                'r--', linewidth=1.5, alpha=0.5, color='#33A89A')
...
# Save figure
output_path = r"C:\Users\13709\Desktop\论文撰写四\模型\训练验证\loss_curves_by_fold.png"
...
ax.plot(epochs_range, train_mean, linewidth=2, color='#FADCAE', label='Training Loss Mean')
ax.fill_between(epochs_range, train_mean - train_std, train_mean + train_std, alpha=0.3, color='#FADCAE')
ax.plot(epochs_range, val_mean, linewidth=2, color='#33A89A', label='Validation Loss Mean')
ax.fill_between(epochs_range, val_mean - val_std, val_mean + val_std, alpha=0.3, color='#33A89A')
...
avg_output_path = r"C:\Users\13709\Desktop\论文撰写四\模型\训练验证\average_loss_curves.png"
