...
def msms_fragment_ablation(
    ...
    nl_mask = torch.ones_like(nl_x[:, :, 0], dtype=torch.bool)
    ...
    for k in range(K):
        ...
        importances[k] = base_logit - logit_k
...
for sample_idx in high_conf_indices:
    ...
    imp = msms_fragment_ablation(model, msms_x, nl_x, precursor_x)

    msms_np = msms_x.cpu().numpy()
    intensity = msms_np[:, 1]
    valid = intensity > 0
    imp_valid = imp[valid]
...
df_samples.to_csv(
    "PBT_msms_fragment_importance_sample_level.csv",
    index=False
)

print("Saved: PBT_msms_fragment_importance_sample_level.csv")
...
df_frags.to_csv(
    "PBT_msms_fragment_importance_fragment_level.csv",
    index=False
)

print("Saved: PBT_msms_fragment_importance_fragment_level.csv")