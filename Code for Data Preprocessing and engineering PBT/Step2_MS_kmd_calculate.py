...
for fname in os.listdir(base_dir):
    ...
    try:
        ...
        # --- Calculate Exact_mass if missing ---
        if "Exact_mass" not in df.columns:
            if "Precursor_mz" not in df.columns:
                print(f"Skipped {fpath} (no Exact_mass or Precursor_mz)")
                continue
            df["Precursor_mz"] = pd.to_numeric(df["Precursor_mz"], errors="coerce")
            if "POS" in fname.upper():
                df["Exact_mass"] = df["Precursor_mz"] - H_mass
            elif "NEG" in fname.upper():
                df["Exact_mass"] = df["Precursor_mz"] + H_mass
            else:
                print(f"Cannot determine POS/NEG mode: {fpath}")
                continue
        ...
        # --- Save updated CSV (to separate folder) ---
