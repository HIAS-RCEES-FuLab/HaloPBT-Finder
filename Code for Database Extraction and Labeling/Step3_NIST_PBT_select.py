...
def filter_compounds(file_pattern, input_csv):
    ...
    for file in files:
        ...
        df = pd.read_csv(file)
        nist_total_rows += len(df)
        if "SMILES" not in df.columns:
            print(f"File {file} has no SMILES column, skipping")
            continue
    ...
    match_info = {
        "nist_files": len(files),
        "nist_total_rows": nist_total_rows,
...
pos_pattern = os.path.join("MSMS_extract", "nist_data_POS_*.csv")
neg_pattern = os.path.join("MSMS_extract", "nist_data_NEG_*.csv")
...
pos_PBT.to_csv("NIST_POS_PBT.csv", index=False)
pos_noPBT.to_csv("NIST_POS_noPBT.csv", index=False)

neg_PBT = neg_match[neg_match["PBT_label"] == 1]
neg_noPBT = neg_match[neg_match["PBT_label"] == 0]
neg_PBT.to_csv("NIST_NEG_PBT.csv", index=False)
neg_noPBT.to_csv("NIST_NEG_noPBT.csv", index=False)

print("\n================= NIST Matching Report =================")
print("\n>>> Input PBT Table Info")
print(f"Total compounds: {input_info['total_rows']}")
print(f"PBT = 1 count: {input_info['PBT_1']}")
print(f"PBT = 0 count: {input_info['PBT_0']}")

print("\n>>> POS Matching Results")
print(f"NIST files: {pos_info['nist_files']}")
print(f"NIST total rows: {pos_info['nist_total_rows']}")
print(f"Matched rows (including duplicates): {pos_info['matched_rows']}")
print(f"PBT = 1 (row-level): {pos_info['PBT_1_rows']}")
print(f"PBT = 0 (row-level): {pos_info['PBT_0_rows']}")
...
print("\n>>> NEG Matching Results")
print(f"NIST files: {neg_info['nist_files']}")
print(f"NIST total rows: {neg_info['nist_total_rows']}")
print(f"Matched rows (including duplicates): {neg_info['matched_rows']}")
print(f"PBT = 1 (row-level): {neg_info['PBT_1_rows']}")
print(f"PBT = 0 (row-level): {neg_info['PBT_0_rows']}")
...
print("\n>>> NIST matching completed. PBT / noPBT files exported successfully!")