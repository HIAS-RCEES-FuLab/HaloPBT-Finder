...
def filter_compounds(file_pattern, input_csv):
    ...
    matched_df = matched_df.merge(
        input_df[["Canonical_smiles", "PBT_label", "Overlap_PBT", "Table2_PBT", "Source_files"]],
        left_on="SMILES",
    ...
    match_info = {
        "gnps_files": len(files),
        "gnps_total_rows": gnps_total_rows,
...
pos_PBT.to_csv("GNPS_POS_PBT.csv", index=False)
pos_noPBT.to_csv("GNPS_POS_noPBT.csv", index=False)

neg_PBT = neg_match[neg_match["PBT_label"] == 1]
neg_noPBT = neg_match[neg_match["PBT_label"] == 0]

neg_PBT.to_csv("GNPS_NEG_PBT.csv", index=False)
neg_noPBT.to_csv("GNPS_NEG_noPBT.csv", index=False)

print("\n================= GNPS Matching Report =================")
...
print(f"GNPS files: {pos_info['gnps_files']}")
print(f"GNPS total rows: {pos_info['gnps_total_rows']}")
...
print(f"GNPS files: {neg_info['gnps_files']}")
print(f"GNPS total rows: {neg_info['gnps_total_rows']}")
...
print("\n>>> Matching completed. GNPS PBT / noPBT files successfully exported!")